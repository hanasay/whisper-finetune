...
add_arg("full_param",    type=bool, default=False,                     help="是否使用lora訓練")
...
add_arg("warmup_steps",  type=int, default=100,     help="训练预热步数")
...
add_arg("learning_rate", type=float, default=5e-6,  help="学习率大小")
add_arg("min_audio_len", type=float, default=0.2,   help="最小的音频长度，单位秒")
...
add_arg('wandb_run_name',            type=str,  default='whisper-hokkien', help='WandB experiment name')
args = parser.parse_args()
print_arguments(args)

# 設定wandb專案名稱
os.environ['WANDB_PROJECT'] = "Hokkien_ASR"

# 初始化processor作為資料處理與tokenizer feature_extractor預處理使用
...
def compute_metrics(pred):
    ...
    with open(f'{args.output_dir}/steps_{len(save_step)*args.eval_steps}-eval_cer', 'w', encoding='utf-8') as file:
        for ref, pred in zip(label_str, pred_str):
            file.write(f"Ref : {ref}\n")
            file.write(f"Pred: {pred}\n")
            file.write(f"Cer : {calc_cer.compute(predictions=[pred], references=[ref])}\n\n")
        save_step.append(0)
        cer = 100 * metric.compute(predictions=pred_str, references=label_str)
        file.write(f"Result cer: {cer}")
...
def main():
    ...
    training_args = \
        Seq2SeqTrainingArguments(
            output_dir=args.output_dir,  # 保存检查点和意志的目录
            ...
            report_to=["wandb"],  # 指定使用tensorboard or wandb保存log
            run_name=args.wandb_run_name,  # name of the W&B run (optional)
            save_steps=args.save_steps,  # 指定保存检查点的步数
            eval_steps=args.eval_steps,  # 指定评估模型的步数
            save_total_limit=5,  # 只保存最新检查点的数量
            optim='adamw_bnb_8bit',  # 指定优化方法
    ...
    if not args.full_param:
        ...
        print('=' * 45, end='')
        print('RUNNING FULL PARAMETER TRAINING', end='')
        print('=' * 45)
    ...
    trainer = Seq2SeqTrainer(
        ...
        compute_metrics=compute_metrics,  # GPU夠用再打開
